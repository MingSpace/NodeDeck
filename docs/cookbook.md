...
**机场自带的 host(DNS 防污染)会自动带出**:不少机场在订阅里放了 Clash 顶层 `hosts:` / Surge `[Host]` 段,把节点域名(如 `*.example.com`)指向自家 DoH 以规避污染。只要节点源「节点源 Host」区的 **emit_hosts 开关开着**(默认开),这些 host 会在**每次刷新时自动解析**并随生成的订阅一并带出,无需手动录入;编辑页该区有只读预览能看到解析到了哪些。手动追加的 host 会与自动解析的去重合并。关掉 emit_hosts 则该源的 host(自动 + 手动)都不带出。

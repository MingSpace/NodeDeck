...
describe("isInfoNode", () => {
  describe("典型信息节点(应识别为 info,过滤掉)", () => {
    const POSITIVE_NAMES = [
      ...
      "45.35 GB | 200 GB", // 无关键字前缀的纯用量行
      "0.00 G | 200.00 G", // 部分机场用 G 而非 GB
      "100GB / 200GB",
      "📊 59.17 GB | 150 GB",
      // 到期类
      "Expire: 2027-05-02",
      ...
      "Expire Date： 2027/04/29", // "Expire" 后跟 "Date" 而非分隔符,需要组合关键字
      "ExpireDate: 2027/04/29",
      // 重置类
      "距离下次重置剩余:25 天",
      "下次重置: 2026-06-01",
      "Reset In: 25 days",
...
🇭🇰 香港 IEPL 专线 1 = trojan, s.com, 443, password=pw`;
  ...
  it("Surge anytls:机场 G 单位 + Traffic Reset + Expire Date 伪节点被剔除", () => {
    const text = `0.00 G | 200.00 G = anytls, e7598a3f37.example.com, 6660, password=anytls-pwd, sni=cache-proxy.example.com, skip-cert-verify=true, tls=true
Traffic Reset：30 Days Left = anytls, e7598a3f37.example.com, 6660, password=anytls-pwd, sni=cache-proxy.example.com, skip-cert-verify=true, tls=true
Expire Date：2099/12/31 = anytls, e7598a3f37.example.com, 6660, password=anytls-pwd, sni=cache-proxy.example.com, skip-cert-verify=true, tls=true
🇭🇰 香港 01 = anytls, e7598a3f37.example.com, 6660, password=anytls-pwd, sni=cache-proxy.example.com, skip-cert-verify=true, tls=true`;
    const out = parseSubscription(text);
    expect(out).toHaveLength(1);
    expect(out[0].name).toBe("🇭🇰 香港 01");
  });

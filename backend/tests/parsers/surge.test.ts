...
[Proxy]
DIRECT = direct
🇭🇰 HK-Trojan = trojan, gz.example.com, 12101, password=secret, sni=m.ctrip.com, skip-cert-verify=true, tfo=true, udp-relay=true
🇭🇰 HK-SS-2022 = ss, 8d4a2926.example.com, 15017, encrypt-method=2022-blake3-aes-128-gcm, password="fake-ss-pwd==", tfo=true, udp-relay=true
...
`;
  ...
  it("parses AnyTLS with server-cert-fingerprint-sha256 (cert pinning)", () => {
    const node = parseSurgeProxyLine(
      "🇭🇰 HK = anytls, 8d4a2926.example.com, 15026, password=pwd, tfo=true, sni=ixigua.com, server-cert-fingerprint-sha256=fac26f65c034829da42d740d23c4a7202475a3834f0ebaecae5f934adbbfd640",
    );
    expect(node).toMatchObject({
      type: "anytls",
      sni: "ixigua.com",
...
describe("parseSurgeConf - bare proxy lines (no [Proxy] header)", () => {
  ...
  it("parses single bare trojan line (bare, no [Proxy] header)", () => {
    const text =
      "🇨🇳 Taiwan 04 = trojan, 8dc9ef6261.example.com, 21409, password=trojan-pwd, sni=v-thumb.byteimg.com, skip-cert-verify=true, tfo=true, udp-relay=true";
    const nodes = parseSurgeConf(text);
    expect(nodes).toHaveLength(1);
    expect(nodes[0]).toMatchObject({
      type: "trojan",
      name: "🇨🇳 Taiwan 04",
      server: "8dc9ef6261.example.com",
      port: 21409,
      password: "trojan-pwd",

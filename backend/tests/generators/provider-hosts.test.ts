...
function provider(emit_hosts: boolean) {
  return {
    ...
    data: providerSchema.parse({
      ...
      name: "Backup",
      type: "http",
      url: "https://sub.example.com/sub?token=x&type=surge",
      emit_hosts,
    }),
  };
}
...
const EXTRACTED = {
  "*.example.com": [
    "server:https://doh1.example:44443/dns-query/x",
    "server:https://doh2.example:44443/dns-query/x",
  ],
};
...
describe("resolveProfile - 上游自动解析的 hosts 随 emit_hosts 带出", () => {
  it("emit_hosts=true 时,cache.extracted_hosts 并入 resolved.hosts", async () => {
    ...
    expect(resolved.hosts?.["*.example.com"]).toEqual(EXTRACTED["*.example.com"]);
  });

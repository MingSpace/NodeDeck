...
export function generateSurgeConfig(input: SurgeGenerateInput): string {
  ...
  lines.push("# Generated by MConvert");
  lines.push(`# Profile: ${profile.id}`);
  lines.push(`# Generated at: ${new Date().toISOString()}`);
  for (const w of input.warnings) lines.push(`# WARN: ${w}`);
  lines.push("");
...
function buildSurgeProxyLine(node: Node, warnings: string[]): string | null {
  ...
  switch (node.type) {
    ...
    case "hysteria2":
      if (node.password !== undefined) params.push(`password=${escapeValue(node.password)}`);
      if (node.up) params.push(`upload-bandwidth=${stripBandwidthUnit(node.up)}`);
    ...
    case "tuic":
      ...
      break;
    case "wireguard":
      if (node.private_key) params.push(`private-key=${node.private_key}`);
      if (node.public_key) params.push(`public-key=${node.public_key}`);
      if (node.preshared_key) params.push(`preshared-key=${node.preshared_key}`);
      if (node.ip) params.push(`self-ip=${node.ip}`);
      if (node.ipv6) params.push(`self-ip-v6=${node.ipv6}`);
      if (node.mtu !== undefined) params.push(`mtu=${node.mtu}`);
      break;
    case "snell":

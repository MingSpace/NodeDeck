...
//
// 识别算法(两类):
//   1) `<LEAD><KEYWORD><STRONG_SEP|WEAK_SEP>`
...
//                     例如 `Traffic: 100 GB` `到期:2027-05-02`。
//      - WEAK_SEP = `- = ~ ·` 等,必须两侧都带空白,避免误伤 `Notice-Premium-01` / `expired-policy`。
//   2) `<LEAD><已用GB|MB|TB><分隔符><总量GB|MB|TB>` —— 无关键字的纯用量行,
//      例如 `45.35 GB | 200 GB`。要求整行仅含装饰前缀 + 两段流量数字,避免误伤真节点名。
//
...
const TRAFFIC_UNIT = String.raw`(?:GB|MB|TB|G)`;
const TRAFFIC_AMOUNT = String.raw`\d+(?:\.\d+)?\s*${TRAFFIC_UNIT}`;
const PURE_TRAFFIC_SEP = String.raw`[|/／\u2502\u2503\uFF0F]`;
// 纯用量行:无 Traffic/流量 等关键字,整行就是 `45.35 GB | 200 GB` 这类形态。
const PURE_TRAFFIC_REGEX = new RegExp(
  `${LEAD}${TRAFFIC_AMOUNT}\\s*${PURE_TRAFFIC_SEP}\\s*${TRAFFIC_AMOUNT}\\s*$`,
  "i",
);
...
export function isInfoNode(node: Pick<Node, "name">): boolean {
  ...
  if (PURE_TRAFFIC_REGEX.test(name)) return true;
